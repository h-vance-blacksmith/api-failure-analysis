...
STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BASE_URL # Target environment/login" -d '{"username": "admin", "password": "wrong"}')
...
STATUS=$(curl -s -o /dev/null -w "%{http_code}" -H "Authorization: Bearer invalid" "$BASE_URL # Target environment/api/v1/data")
...
STATUS=$(curl -s -o /dev/null -w "%{http_code}" "$BASE_URL # Target environment/api/v1/resource/999")
...
STATUS=$(curl -s -o /dev/null -w "%{http_code}" "$BASE_URL # Target environment/api/v1/trigger-error")
